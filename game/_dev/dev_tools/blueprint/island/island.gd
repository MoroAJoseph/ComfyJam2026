@tool
class_name VoxelBlueprintIsland
extends Node3D
...
@export var blueprint_data: VoxelBlueprintIslandData
...
@export var spawn_rules: Array[VoxelBlueprintVoxelSpawnRule] = []
...
func generate_blueprint() -> void:
	...
	# Create a deterministic RNG stream
...
func _heightmap_to_voxels_with_caves(
	...
	return VoxelBlueprintIslandGenerator.generate_island_voxels(
		heightmap, 
		dims, 
		sea_h, 
		rng, 
...
func bake_to_blueprint_data() -> void:
	...
	blueprint_data.block_map = voxel_data.duplicate()
	blueprint_data.emit_changed()
