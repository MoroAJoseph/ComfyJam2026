...
func _ready() -> void:
	...
	# Initial gold update
	_update_gold_label(Session.player_context.gold)
	_update_upgrade_ui(Session.player_context.equipped_boat)
	
	# Listen for gold updates
	Session.player_context.gold_updated.connect(_update_gold_label)
	Session.player_context.equipped_boat_updated.connect(_update_upgrade_ui)
...
func _update_gold_label(value: int) -> void:
	...
	_update_upgrade_button_label(Session.player_context.equipped_boat)
